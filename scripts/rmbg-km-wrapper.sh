...
process_file() {
    ...
    # Pass filtered arguments from the wrapper call directly to the rmbg binary
    if "$RMBG" "${rmbg_args[@]}" "$input" >> "$LOG" 2>&1 && [[ -f "$rmbg_result" ]]; then
        if mv "$rmbg_result" "$out" >> "$LOG" 2>&1; then
            converted=$((converted + 1))
            echo "Wrote: $out" >> "$LOG"
            if [[ "$replace" == true && "$src" != "$out" ]]; then
                rm -f "$src" >> "$LOG" 2>&1
            fi
        else
